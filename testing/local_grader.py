...
def compare_with_mask(student_out, reference_out, reference_mask, filename):
    ...
    while True:
        ...
        masked_line1 = "".join(
            [
                line1[i] if mask_line[i].strip() == "1" else " "
                for i in range(len(line1))
            ]
        )
        masked_line2 = "".join(
            [
                line2[i] if mask_line[i].strip() == "1" else " "
...
tests = [
    (
        "Testcase 0 (Local test 0)",
        "student_output/p22_top_localtest_0.csv",
        TestCase(
            os.path.join(file_locations, "circ_files/p22_localtest_0.circ"),
            os.path.join(file_locations, "reference_output/proj_2_2_localtest_0.csv"),
            "",
            "",
        ),
    ),
    (
        "Testcase 1 (Local test 1)",
        "student_output/p22_top_localtest_1.csv",
        TestCase(
            os.path.join(file_locations, "circ_files/p22_localtest_1.circ"),
            os.path.join(file_locations, "reference_output/proj_2_2_localtest_1.csv"),
            "",
            "",
        ),
    ),
